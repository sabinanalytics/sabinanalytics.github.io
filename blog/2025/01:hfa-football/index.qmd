...
library(lme4)
#bayesian model 
epa_game_model <- lmer(epa_per_70 ~ -1 + #no intercept since I made them all mean 0
...
nfl_penalty_game_summary <- nfl_penalty_game_summary |> 
  ...
  mutate(penalty_yards_per_100_plays = penalty_yards / plays * 100)

penalty_model <- lm(penalty_yards_per_100_plays ~ posteam_site +
                      bs(team_rest_effect, df = 3) +
                      indoors +
                      grass +
                      team_pred_pt_diff + 
...
```{r}
penalty_model2 <- lm(penalty_yards_per_100_plays ~ posteam_site +
                      team_pred_pt_diff + 
                       total_line,
                    data = nfl_penalty_game_summary)
...
```{r}
referee_penalty_model <- lmer(penalty_yards_per_100_plays ~ posteam_site +
                              (1|referee) +
                      team_pred_pt_diff + 
                       total_line,
                    data = nfl_penalty_game_summary)
...
referee_effects <- tidy(referee_penalty_model, effect = "ran_vals") |> 
  ...
  rename(yards_per_100 = estimate,
         referee = level)

referee_effects |> 
  slice(1:10) |> 
  dplyr::select(referee, yards_per_100, std.error) |> 
  gt() |>
  cols_label(
    referee = "Referee",
    yards_per_100 = "Penalty Yards per 100 Plays",
    std.error = "Standard Error"
  ) |> 
  #round last 2 columns to 2 digits
  fmt_number(
